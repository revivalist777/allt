...
extern UniValue DozalltSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzalltseed(const UniValue& params, bool fHelp);
extern UniValue getzalltseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzallt(const UniValue& params, bool fHelp);
extern UniValue dzalltstate(const UniValue& params, bool fHelp);
