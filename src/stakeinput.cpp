...
CzalltStake::CzalltStake(const libzerocoin::CoinSpend& spend)
{
...
int CzalltStake::GetChecksumHeightFromMint()
{
...
int CzalltStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CzalltStake::GetChecksum()
{
    return nChecksum;
}

// The zallt block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CzalltStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CzalltStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zallt staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CzalltStake::GetUniqueness()
{
    //The unique identifier for a zallt is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CzalltStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CzalltStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zallt that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreatezalltOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zallt output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zallt", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreatezalltOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zallt output", __func__);
...
bool CzalltStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CzalltStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzalltTracker* zalltTracker = pwallet->zalltTracker.get();
    CMintMeta meta;
    if (!zalltTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zalltTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!allt Stake
bool CalltStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CalltStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CalltStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CalltStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CalltStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CalltStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CalltStake::GetUniqueness()
{
    //The unique identifier for a allt stake is the outpoint
...
CBlockIndex* CalltStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
