...
public:
    ...
    bool WritezalltSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadzalltSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadzalltSeed_deprecated(uint256& seed);
    bool ErasezalltSeed();
    bool ErasezalltSeed_deprecated();

    bool WritezalltCount(const uint32_t& nCount);
    bool ReadzalltCount(uint32_t& nCount);
