...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "allt" is a composite category enabling all allt-related debug output
            if (ptrCategory->count(std::string("allt"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\allt
// Windows >= Vista: C:\Users\Username\AppData\Roaming\allt
// Mac: ~/Library/Application Support/allt
// Unix: ~/.allt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "allt";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "allt.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty allt.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override allt.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "alltd.pid"));
