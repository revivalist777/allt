// Copyright (c) 2018-2019 The allt developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zallt/deterministicmint.h>
#include "zallttracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzalltTracker::CzalltTracker(std::string strWalletFile)
{
...
CzalltTracker::~CzalltTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzalltTracker::Init()
{
...
bool CzalltTracker::Archive(CMintMeta& meta)
{
...
bool CzalltTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzalltTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzalltTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzalltTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzalltTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzalltTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzalltTracker::GetSerialHashes()
{
...
CAmount CzalltTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzalltTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzalltTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzalltTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzalltTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzalltTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzalltTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzalltTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzalltTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzalltTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzalltTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzalltWallet* zalltWallet)
{
    bool iszalltWalletInitialized = (NULL != zalltWallet);
    CMintMeta meta;
    ...
    if (! iszalltWalletInitialized)
        zalltWallet = new CzalltWallet(strWalletFile);
    meta.isSeedCorrect = zalltWallet->CheckSeed(dMint);
    if (! iszalltWalletInitialized)
        delete zalltWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzalltTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzalltTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzalltTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzalltTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzalltTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzalltTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzalltWallet* zalltWallet = new CzalltWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zalltWallet);
        }
        delete zalltWallet;
        LogPrint("zero", "%s: added %d dzallt from DB\n", __func__, listDeterministicDB.size());
    }
