...
CzalltWallet::CzalltWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zallt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzalltSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezalltSeed_deprecated()) {
                    LogPrintf("%s: Updated zallt seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zallt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zallt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzalltWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezalltCount(nCountLastUsed);
    else if (!walletdb.ReadzalltCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzalltWallet::Lock()
{
    seedMaster = 0;
}

void CzalltWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzalltWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozallt(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzalltWallet::LoadMintPoolFromDB()
{
...
void CzalltWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzalltWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzalltWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zalltTracker->HasPubcoinHash(pMint.first)) {
...
bool CzalltWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozallt(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zalltTracker which also adds to database
    pwalletMain->zalltTracker->Add(dMint, true);
...
void CzalltWallet::SeedTozallt(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzalltWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzalltWallet::GenerateDeterministiczallt(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzalltWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzalltWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzalltWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
