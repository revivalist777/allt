...
#include "qt/allt/forms/ui_addresseswidget.h"
#include "qt/allt/addresslabelrow.h"
#include "qt/allt/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(alltGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a allt address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress alltAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(alltAdd.Get(), label.toUtf8().constData(),
                alltAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
