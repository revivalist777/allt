...
#include "qt/allt/topbar.h"
#include "qt/allt/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/allt/lockunlock.h"
#include "qt/allt/qtutils.h"
#include "qt/allt/receivedialog.h"
...
TopBar::TopBar(alltGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopallt, ui->labelAmountTopzallt}, "amount-small-topbar");
    setCssProperty({ui->labelAmountallt, ui->labelAmountzallt}, "amount-topbar");
    setCssProperty({ui->labelPendingallt, ui->labelPendingzallt, ui->labelImmatureallt, ui->labelImmaturezallt}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzallt->setVisible(false);
    ui->labelAmountzallt->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzallt->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezallt->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount alltAvailableBalance = balance + delegatedBalance;

    // zallt Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalallt = GUIUtil::formatBalance(alltAvailableBalance, nDisplayUnit);
    QString totalzallt = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopallt->setText(totalallt);
    ui->labelAmountTopzallt->setText(totalzallt);

    // Expanded
    ui->labelAmountallt->setText(totalallt);
    ui->labelAmountzallt->setText(totalzallt);

    ui->labelPendingallt->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzallt->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureallt->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezallt->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
