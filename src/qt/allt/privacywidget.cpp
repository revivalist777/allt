...
#include "qt/allt/privacywidget.h"
#include "qt/allt/forms/ui_privacywidget.h"
#include "qt/allt/qtutils.h"
#include "guiutil.h"
#include "qt/allt/denomgenerationdialog.h"
#include "qt/allt/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(alltGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zallt anonymizes your allt by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zallt or convert back to allt"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 allt ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zallt");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zallt");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zallt");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zallt");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zallt");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zallt");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zallt");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zallt");
    ...
    // Only Convert to allt enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzallt->setTitleClassAndText("btn-title-grey", "Total 0 zallt");
    ui->btnTotalzallt->setSubTitleClassAndText("text-subtitle", "Show denominations of zallt owned.");
    ui->btnTotalzallt->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select allt outputs to mint into zallt.");
    ...
    connect(ui->btnTotalzallt, SIGNAL(clicked()), this, SLOT(onTotalzalltClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zallt");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of allt to mint into zallt"));
    }else{
        btnText = tr("Convert back to allt");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zallt to convert back into allt"));
    }
...
void PrivacyWidget::onTotalzalltClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzallt->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzallt->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zallt is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zallt, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zallt").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zallt minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackzallt(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zallt converted back to allt"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zallt </b>";
    ...
    ui->btnTotalzallt->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
