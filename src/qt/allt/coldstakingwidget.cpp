...
#include "qt/allt/forms/ui_coldstakingwidget.h"
#include "qt/allt/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/allt/requestdialog.h"
...
ColdStakingWidget::ColdStakingWidget(alltGUI* parent) :
    PWidget(parent),
    ui(new Ui::ColdStakingWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your allts and let a hot node (24/7 online node)\nstake in your behalf, keeping the keys in a secure place offline."));
    ...
    ui->labelSubtitle2->setText(tr("Delegate or Accept allt delegation"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select allt outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 allt" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
