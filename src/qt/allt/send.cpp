...
#include "qt/allt/send.h"
#include "qt/allt/forms/ui_send.h"
#include "qt/allt/addnewcontactdialog.h"
#include "qt/allt/qtutils.h"
#include "qt/allt/sendchangeaddressdialog.h"
#include "qt/allt/optionbutton.h"
#include "qt/allt/sendconfirmdialog.h"
#include "qt/allt/myaddressrow.h"
#include "qt/allt/guitransactionsutils.h"
...
SendWidget::SendWidget(alltGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("allt");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zallt");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (allt)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a allt address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 allt");
    ...
    setCssProperty(coinIcon, "coin-icon-allt");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onalltSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onalltSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send allt");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zallt");
...
void SendWidget::refreshAmounts() {
    ...
    bool iszallt = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, iszallt));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (iszallt ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendallt = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendallt ? AskPassphraseDialog::Context::Send_allt : AskPassphraseDialog::Context::Send_zallt, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendallt) ? send(recipients) : sendzallt(recipients)) {
...
bool SendWidget::sendzallt(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zallt is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zallt selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!zalltControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zalltControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendzallt(
        ...
        inform(tr("zallt transaction sent!"));
        zalltControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == zallt_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zallt require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zallt transaction
        ...
        emit message("zallt transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isallt){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any allt to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            zalltControlDialog *zalltControl = new zalltControlDialog(this);
            zalltControl->setModel(walletModel);
            zalltControl->exec();
            ui->btnCoinControl->setActive(!zalltControlDialog::setSelectedMints.empty());
            zalltControl->deleteLater();
        } else {
            inform(tr("You don't have any zallt in your balance to select."));
        }
    }
}
...
void SendWidget::onalltSelected(bool _isallt){
    isallt = _isallt;
    setCssProperty(coinIcon, _isallt ? "coin-icon-allt" : "coin-icon-zallt");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress alltAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(alltAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(alltAdd.Get(), dialog->getLabel().toStdString(), "send")) {
