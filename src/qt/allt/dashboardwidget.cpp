...
#include "qt/allt/dashboardwidget.h"
#include "qt/allt/forms/ui_dashboardwidget.h"
#include "qt/allt/sendconfirmdialog.h"
#include "qt/allt/txrow.h"
...
DashboardWidget::DashboardWidget(alltGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of allt staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareallt, "square-chart-allt");
    setCssProperty(ui->labelSquarezallt, "square-chart-zallt");
    setCssProperty(ui->labelallt, "text-chart-allt");
    setCssProperty(ui->labelzallt, "text-chart-zallt");
    ui->labelzallt->setVisible(false);
    ui->labelSquarezallt->setVisible(false);
    ...
    ui->labelAmountzallt->setText("0 zallt");
    ui->labelAmountzallt->setVisible(false);
    ui->labelAmountallt->setText("0 allt");
    setCssProperty(ui->labelAmountallt, "text-stake-allt-disable");
    setCssProperty(ui->labelAmountzallt, "text-stake-zallt-disable");
    ...
    ui->btnHowTo->setText(tr("How to get allt"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("allt")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isallt = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::Stakezallt;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair allt, zallt
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            allt = (pair.first != 0) ? pair.first / 100000000 : 0;
            zallt = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalallt += pair.first;
            chartData->totalzallt += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesallt.append(allt);
        chartData->valueszallt.append(zallt);

        int max = std::max(allt, zallt);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("allt");
    set1 = new QBarSet("zallt");
    ...
    set0->append(chartData->valuesallt);
    set1->append(chartData->valueszallt);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalallt > 0 || chartData->totalzallt > 0) {
        setCssProperty(ui->labelAmountallt, "text-stake-allt");
        setCssProperty(ui->labelAmountzallt, "text-stake-zallt");
    } else {
        setCssProperty(ui->labelAmountallt, "text-stake-allt-disable");
        setCssProperty(ui->labelAmountzallt, "text-stake-zallt-disable");
    }
    forceUpdateStyle({ui->labelAmountallt, ui->labelAmountzallt});
    ui->labelAmountallt->setText(GUIUtil::formatBalance(chartData->totalallt, nDisplayUnit));
    ui->labelAmountzallt->setText(GUIUtil::formatBalance(chartData->totalzallt, nDisplayUnit, true));
