...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::allt);
    ...
    if (settings.contains("nAnonymizealltAmount") || reset)
        SoftSetArg("-anonymizealltamount", settings.value("nAnonymizealltAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in allt.cpp during shotdown to also remove the window positions
