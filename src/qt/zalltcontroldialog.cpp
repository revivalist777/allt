...
#include "ui_zalltcontroldialog.h"
...
std::set<std::string> zalltControlDialog::setSelectedMints;
std::set<CMintMeta> zalltControlDialog::setMints;

bool CzalltControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zalltControlDialog::COLUMN_DENOMINATION || column == zalltControlDialog::COLUMN_VERSION || column == zalltControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zalltControlDialog::zalltControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zalltControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zallt Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelzallt->setProperty("cssClass", "text-main-purple");
    ui->labelzallt_int->setProperty("cssClass", "text-main-purple");
...
zalltControlDialog::~zalltControlDialog()
{
    delete ui;
}

void zalltControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zalltControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzalltControlWidgetItem* itemDenom(new CzalltControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzalltControlWidgetItem *itemMint = new CzalltControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zalltTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zalltTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zallt.";
            else if (!mint.isSeedCorrect)
                strReason = "The zallt seed used to mint this zallt is not the same as currently hold in the wallet";
            else
...
void zalltControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zalltControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzallt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setzalltControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zalltControlDialog::GetSelectedMints()
{
...
void zalltControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
