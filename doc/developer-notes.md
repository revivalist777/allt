...
- [Development guidelines](#development-guidelines)
    - [General allt Coin](#general-allt-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
allt Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play allts (tallt)" on the test network, if you
...
allt Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/alltransfercoin/allt/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_allt
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_allt --log_level=test_suite
$ valgrind -v --leak-check=full src/alltd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_allt.coverage/index.html`.
```
...
- BitcoinMiner : Generates allts (if wallet is enabled).
...
However, in open source software such as allt Coin, where everyone uses
...
pay attention to for reviewers of allt Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: allt Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against allt Coin so that they can be integrated
...
file descriptors it will cause problems with allt's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:allt/allt.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `allt-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `allt-cli` and the GUI debug console use this table to determine how to
