...
*Setup instructions for a gitian build of allt Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the allt
...
to the allt GitHub release page.
...
- [Building allt Coin](#building-allt-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/allt-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/allt-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/allt-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/allt-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/allt-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/allt-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/alltransfercoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@alltcoin.online) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/allt-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/allt-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/allt-osx-*-build.assert
```
